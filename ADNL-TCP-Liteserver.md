...
## Data exchange
After we have established a connection, we can start sending and receiving data; the TL language is used to serialize data.
...
After successfull handshake, client should start authentification, for liteserver any randomly generated key can be used, but for some services specific keys are needed. (example: storage-daemon or validator control).

1. Client should send TL message with scheme `tcp.authentificate nonce:bytes = tcp.Message`, where nonce is 1 - 512 random bytes. 
2. Server will respond with `tcp.authentificationNonce nonce:bytes = tcp.Message` which will contain random bytes generated by the server.
3. Now you should concat your nonce with server nonce and sign it with your authentification key
4. Serialize TL struct `tcp.authentificationComplete key:PublicKey signature:bytes = tcp.Message` and send back to server.
5. Server will verify key, signature and nonces and you will be authentificated, now server will be able to respond to your requests.

[Example implementation in tonutils-go](https://github.com/xssnick/tonutils-go/blob/dev-v1-11/liteclient/auth.go)
...
5. `state` - [BoC](/Cells-BoC.md#bag-of-cells) TLB [account state scheme](https://github.com/ton-blockchain/ton/blob/master/crypto/block/block.tlb#L232).

Of all this data, what we need is in the state, we will analyze it. 

For example, let's get the status of TF account `EQAhE3sLxHZpsyZ_HecMuwzvXHKLjYx4kEUehhOy2JmCcHCT`, `state` in the response will be:
```
...
[Parse this BoC](/Cells-BoC.md#bag-of-cells) and get 
...
```
Our structure references others such as::
```
...
```
Convert to binary form and get:
```
...
##### Other methods
Now, having studied all the information, you can call and process responses from other lite-server methods as well. Same principle :)
